    ...
    if not existing:
        ...
        if cr.status_code >= 300:
            if 'parent was not found' in cr.text.lower() or 'parent was not found' in cr.reason.lower():
                m.fail_json(msg=(
                    f"CREATE failed {cr.status_code} parent zone missing. The required DNS zone must be created manually in Infoblox before creating host records. Raw: {cr.text}"))
    ...
    if upd:
        ...
        if ur.status_code >= 300:
            if 'parent was not found' in ur.text.lower():
                m.fail_json(msg=(
                    f"UPDATE failed {ur.status_code} parent zone missing for {name}. The required DNS zone must be created manually in Infoblox. Raw: {ur.text}"))
